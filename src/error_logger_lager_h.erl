...
format_reason({'function not exported', [{M, F, A},MFA|R]}) ->
  format_reason({'function not exported', [{M, F, A, []},MFA|R]});
format_reason({'function not exported', [{M, F, A, L},MFA|_]}) ->
    ["call to undefined function ", format_mfa({M, F, length(A), L}),
        " from ", format_mfa(MFA)];
...
format_reason({system_limit, [{M, F, A}|R]}) ->
    format_reason({system_limit, [{M, F, A, []}|R]});
format_reason({system_limit, [{M, F, _, _}|_] = Trace}) ->
    Limit = case {M, F} of
        {erlang, open_port} ->
    ...
    ["system limit: ", Limit];
format_reason({badarg, [{M,F,A},MFA2|R]}) ->
    format_reason({badarg, [{M,F,A,[]},MFA2|R]});
format_reason({badarg, [MFA,MFA2|_]}) ->
    case MFA of
        {_M, _F, A, _L} when is_list(A) ->
...
format_mfa({M, F, A, []}) ->
    format_mfa({M, F, A});
format_mfa({M, F, A, [{file, File}, {line, Line}]}) ->
