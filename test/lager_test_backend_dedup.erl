        ...
        [
            ...
            {"logging works from inside a list comprehension",
                fun() ->
                        ...
                        ?assertEqual(1, count()),
                        {_Level, _Time, Message}  = pop(),
                        [LevelStr, _LocStr, MsgStr] = re:split(Message, " ", [{return, list}, {parts, 3}]),
                        ?assertEqual("[warning]", LevelStr),
                        ?assertEqual("test message (10 times)", MsgStr),
                        ok
                end
            },
            ...
            {"logging works from a nested list comprehension",
                fun() ->
                        ...
                        [LevelStr, _LocStr, MsgStr] = re:split(Message, " ", [{return, list}, {parts, 3}]),
                        ?assertEqual("[warning]", LevelStr),
                        ?assertEqual("test message (100 times)", MsgStr),
                        ok
                end
            },
        ...
        [
            ...
            {"info reports are truncated at 4096 characters",
                fun() ->
                        ...
                        _ = gen_event:which_handlers(error_logger),
                        %% this somehow takes longer, so we need a bit of
                        %% a sleepy time. Should look into that.
                        timer:sleep(500),
                        {_, _, Msg} = pop(),
